...
class R2Test {
  ...
  @Test
  void getScore() {
    R2 metric = new R2(null); // Recommender not needed;

    assertEquals(1.0, metric.getScore(user, perfectPrediction));
    assertEquals(0.8, metric.getScore(user, slightlyPrefectPrediction));
    assertEquals(-2.4, metric.getScore(user, allMinimumPrediction));
    assertEquals(-2.4, metric.getScore(user, allMaximumPrediction));
  }
}
