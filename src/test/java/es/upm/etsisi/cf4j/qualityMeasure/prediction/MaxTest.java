...
class MaxTest {
  ...
  @Test
  void getScore() {
    Max metric = new Max(null); // Recommender not needed;

    assertEquals(0.0, metric.getScore(user, perfectPrediction));
    assertEquals(1.0, metric.getScore(user, slightlyPrefectPrediction));
    assertEquals(4.0, metric.getScore(user, allMinimumPrediction));
    assertEquals(4.0, metric.getScore(user, allMaximumPrediction));
  }
}
