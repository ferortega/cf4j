...
class MSLETest {
  ...
  @Test
  void getScore() {
    MSLE metric = new MSLE(null); // Recommender not needed;

    assertEquals(0.0, metric.getScore(user, perfectPrediction));
    assertEquals(0.13904563092855698, metric.getScore(user, slightlyPrefectPrediction));
    assertEquals(0.7270769844453228, metric.getScore(user, allMinimumPrediction));
    assertEquals(0.5844437572867733, metric.getScore(user, allMaximumPrediction));
  }
}
